...
<section id="datastream" class="relative w-full py-16">
            ...
            <div class="relative w-full">
                ...
                >
                    {#each carouselData as item, i}
                        <!-- Desktop Layout -->
                        <div
                            class="snap-center w-full flex-shrink-0 hidden lg:flex flex-row gap-0 h-[600px]"
                        >
                            <!-- Text box -->
                            <div
                                class="w-1/2 bg-surface-contrast-50 flex flex-col justify-center p-24"
                            >
                                <div class="space-y-4">
                            ...
                            <!-- Image -->
                            <div
                                class="w-1/2 h-full overflow-hidden flex items-center justify-center"
                            >
                                <img
                        ...
                        <!-- Mobile Layout -->
                        <div
                            class="snap-center w-full flex-shrink-0 flex lg:hidden flex-col gap-0 h-[400px]"
                        >
                            <!-- Text box -->
                            <div class="w-full flex flex-col justify-center p-16 h-full">
                                <div class="space-y-4">
                                    ...
                                    <p
                                        class="text-sm leading-relaxed text-surface-950-50 text-center"
                                    >
                                        {@html $t(item.description)}
                                    </p>
                                </div>
    ...
    <div class="flex justify-center space-x-6 mt-8">
        {#each carouselData as _, i}
            <button
                class="w-3 h-3 rounded-full transition-all duration-200 {currentSlide === i
                    ? 'bg-tertiary-500'
                    : 'bg-surface-950-50/90'}"
                onclick={() => {
                    if (carouselContainer) {
                        carouselContainer.scrollTo({
                            left: carouselContainer.clientWidth * i,
                            behavior: 'smooth',
                        })
                        currentSlide = i
                    }
