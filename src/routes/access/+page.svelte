	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
		<div class="lg:col-span-2 space-y-2">
			<div class="relative">
				...
				/>
				<SearchIcon class="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-surface-800 dark:text-surface-300" />
				{#if searchQuery.trim() !== ''}
					<button
		...
		<div class="order-1 lg:order-2 lg:col-span-1">
			<div class="border border-surface-800 dark:border-surface-600 rounded-lg p-4 space-y-3">
				...
				{#if selectedConcepts.length === 0}
					<p class="text-surface-800 dark:text-surface-300 text-sm">No concepts selected</p>
				{:else}
					<div class="flex flex-wrap gap-2">
						{#each selectedConcepts as key}
							{#if selectedMap[key]}
								<span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-surface-contrast-50 dark:bg-surface-100 text-surface-50 dark:text-surface-900 text-sm">
									{concepts.find((c) => sanitizeKey(c.label) === key)?.label ?? key}
									<button
										class="text-primary-700 hover:text-primary-900"
										onclick={() => removeSelected(key)}
										aria-label="Remove selected concept"
									>
										<XIcon class="w-5 h-5 text-surface-50 dark:text-surface-900 hover:text-primary-200 hover:dark:text-primary-800" />
									</button>
								</span>
							{/if}
						{/each}
		...
		<div class="order-2 lg:order-1 lg:col-span-2 space-y-4 flex flex-col">
			<div class="border border-surface-800 dark:border-surface-600 rounded-lg p-4 flex-1 min-h-[50vh] max-h-[calc(100vh-320px)] overflow-auto">
						...
						{#if concept}
							<label class="flex items-start gap-3 cursor-pointer">
								...
								<div class="space-y-1">
									...
									<div class="text-xs text-surface-900 dark:text-surface-50 leading-snug">
										{concept.definition}
									</div>
								</div>
							</label>
